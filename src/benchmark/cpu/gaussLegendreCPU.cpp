...
namespace BenchMark {
    namespace Cpu {
        void gaussLegendreCPU::initialize(int numberOfDecimals, int noThreads) {
            gaussLegendreCPU::noDecimals = numberOfDecimals;
            gaussLegendreCPU::noThreads = noThreads;
        }
            ...
            for (int t = 0; t < noThreads; ++t) {
                threads[t] = std::thread([&]() {
                    ...
                    int iterations_per_thread = ceil(
                            log2(gaussLegendreCPU::noDecimals / 14.6) /
        ...
        void gaussLegendreCPU::runAbsolute(bool multithreading){
            if (multithreading) {
                gaussLegendreCPU::runMultiThreaded(gaussLegendreCPU::noThreads);
            } else {
